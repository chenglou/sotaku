# The growth stage of the bootstrap-then-grow program. Seed: model_tiny_bootstrap.pt
# (52K params, 5,000 supervised steps — solves 51/1000 at 16 iterations, 228/1000 at
# 128, so ES has slope). ES runs with dense cell fitness at 128 iterations; at
# generation GROW_AT_GEN every feed-forward layer widens d_ff 128 -> 512
# function-preservingly (new first-linear rows are small random values, the matching
# new second-linear input columns are ZERO, so the widened network computes exactly
# the same function and keeps its fitness). The questions: does ES climb from the
# bootstrap at all, and does the grown network climb further or faster than the
# small one was climbing?
...
CHECKPOINT_PREFIX = "es_grow_checkpoint_step"

CONFIG = {
    'experiment': 'exp_es_grow',
    'd_model': 32,
    'n_heads': 4,
    'd_ff_small': 128,
    'd_ff_big': 512,
...
def grown_state_dict(state, d_ff_new):
    """Widen every feed-forward layer function-preservingly: the new first-linear rows
    get small random values (symmetry breaking), the matching new second-linear input
    columns get zeros, so the widened network's output is unchanged."""
    gen = torch.Generator().manual_seed(4242)
    new_state = dict(state)
    for li in range(n_layers):
        w1 = state[f'layers.{li}.linear1.weight']
        b1 = state[f'layers.{li}.linear1.bias']
        w2 = state[f'layers.{li}.linear2.weight']
        extra = d_ff_new - w1.shape[0]
        new_rows = torch.randn(extra, w1.shape[1], generator=gen) * 0.02
        new_state[f'layers.{li}.linear1.weight'] = torch.cat([w1.cpu(), new_rows], dim=0)
        new_state[f'layers.{li}.linear1.bias'] = torch.cat([b1.cpu(), torch.zeros(extra)], dim=0)
        new_state[f'layers.{li}.linear2.weight'] = torch.cat([w2.cpu(), torch.zeros(w2.shape[0], extra)], dim=1)
    return new_state
...
def train(output_dir="."):
    ...
    if checkpoint_path:
        ...
        current_d_ff = int(ckpt.get('d_ff', D_FF_SMALL))
        model = make_model(current_d_ff).to(device)
        model.load_state_dict(ckpt['model_state_dict'])
        start_gen = ckpt['step'] + 1
        log(f"Resumed from generation {ckpt['step']} at d_ff={current_d_ff}")
    else:
        seed_path = os.path.join(os.getcwd(), "seed_model.pt")
        state = torch.load(seed_path, map_location='cpu', weights_only=True)
        if 'model_state_dict' in state:
            state = state['model_state_dict']
        current_d_ff = D_FF_SMALL
        model = make_model(current_d_ff).to(device)
    ...
    log(f"Model: {n_params} parameters (d_ff={current_d_ff})")
    ...
    for gen in range(start_gen, total_steps):
        if gen == GROW_AT_GEN and current_d_ff == D_FF_SMALL:
            pre = score_slice(0)
            grown = grown_state_dict({k: v.cpu() for k, v in model.state_dict().items()}, D_FF_BIG)
            current_d_ff = D_FF_BIG
            model = make_model(current_d_ff).to(device)
            ...
            post = score_slice(0)
            n_params = sum(p.numel() for p in params)
            log(f"GROW at gen {gen}: d_ff {D_FF_SMALL} -> {D_FF_BIG} ({n_params} params) | "
