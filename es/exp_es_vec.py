# Vector-valued ES fitness from scratch: instead of one scalar per population member,
# each member is scored PER PUZZLE (cells correct on each of the 384), and each
# antithetic pair's update weight is the majority vote across puzzles — the mean of
# sign(plus minus minus) per puzzle, in [-1, 1], carrying ~384 comparisons where
# scalar ES carries one. A step toward the per-cell credit assignment identified as
# backprop's remaining monopoly, with the ES machinery otherwise unchanged from the
# flat-at-chance scalar pilot. The question is whether the richer signal finds a slope.
...
CHECKPOINT_PREFIX = "es_vec_checkpoint_step"

CONFIG = {
    'experiment': 'exp_es_vec',
    'd_model': 32,
    'n_heads': 4,
    'd_ff': 128,
    'n_layers': 4,
...
def train(output_dir="."):
    ...
    def score_slice(lo):
        """Per-puzzle cell-correct counts: a vector of 384 scores, not one scalar."""
    ...
    for candidate in sigma_ladder:
        ...
        with torch.no_grad():
            for p, s in zip(model.parameters(), snapshot):
                p.copy_(s)
        log(f"CALIBRATE sigma={candidate:.0e}: {int(score.sum())} (unperturbed {int(unperturbed.sum())})")
        if score.sum() >= unperturbed.sum() // 2:
            sigma = candidate
            break
    ...
    if start_gen >= total_steps:
        ...
        final_path = os.path.join(output_dir, "model_es_vec.pt")
    ...
    for gen in range(start_gen, total_steps):
        ...
        # Per-pair majority vote across puzzles: mean of sign(f+ - f-), in [-1, 1],
        # carrying ~384 comparisons where the scalar version carries one.
        pair_votes = []
        with torch.no_grad():
            for i, seed in enumerate(seeds):
                diff = (scores_plus[i].float() - scores_minus[i].float()).sign()
                w = float(diff.mean().item()) / population_pairs
                pair_votes.append(w)
                if w == 0.0:
                    continue
                ...
                for p in params:
                    ...
                    p.add_(z, alpha=lr * w)
        all_scores = np.array([float(s.sum()) for s in scores_plus + scores_minus])
        vote_mass = float(np.abs(pair_votes).sum())
        ...
        if gen % 10 == 0 or gen == total_steps - 1:
            ...
            log(f"GEN {gen:4d} | fitness mean {np.mean(all_scores):.0f} "
                f"best {max(all_scores):.0f} | vote mass {vote_mass:.3f} | validation {fitness_iters}-iter: "
