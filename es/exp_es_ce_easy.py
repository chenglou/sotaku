# CE fitness from scratch on the EASIEST puzzles only: the 2,048 pool puzzles with
# the fewest empty cells. More givens means more anchored context per prediction, so
# the smooth-fitness slope (which exists — phase one climbed ten nats) may point
# toward actual digit prediction instead of the uniform shrug. A curriculum would
# then widen the pool as fitness moves; this probe only tests the first rung.
...
CHECKPOINT_PREFIX = "es_ce_easy_checkpoint_step"

CONFIG = {
    'experiment': 'exp_es_ce_easy',
    'd_model': 32,
    'n_heads': 4,
    'd_ff': 128,
    'n_layers': 4,
...
fitness_pool_size = 2_048   # after the easiest-slice filter below
log_name = "exp_es_ce_easy.log"
...
def train(output_dir="."):
    ...
    # Keep only the easiest slice: fewest empty cells first.
    order = pool_empty.sum(dim=1).argsort()[:2048]
    pool_x = pool_x[order]
    pool_targets = pool_targets[order]
    pool_empty = pool_empty[order]
    print(f"Fitness pool: {pool_x.size(0)} easiest puzzles "
          f"(empties {int(pool_empty.sum(dim=1).min())}-{int(pool_empty.sum(dim=1).max())})")
    ...
    if start_gen >= total_steps:
        ...
        final_path = os.path.join(output_dir, "model_es_ce_easy.pt")
