# The generality test: mazes through the UNCHANGED pixel sudoku architecture. Same
# 81 tokens, same 2D RoPE, same iterative refinement, same pixel-in/pixel-out
# contract — the only new code is a renderer (wall/floor/start/goal glyphs) and a
# maze generator. The model sees a maze as bitmaps and must draw the unique shortest
# path into the floor cells. If iteration scaling appears here too (128-iter probe
# above 16-iter), the "general 2D iterative solver" claim gains its second task
# without a single architecture change.
#
# Mazes: 9x9, random walls (density 0.28), random start/goal, rejection-sampled for
# a unique shortest path of length >= 6 so exact-match solving is well-defined.
...
CHECKPOINT_PREFIX = "pixel_maze_checkpoint_step"

CONFIG = {
    'experiment': 'exp_pixel_maze',
    'd_model': 64,
    'n_heads': 8,
    'd_ff': 256,
    'n_layers': 4,
...
def train(output_dir="."):
    ...
    if start_step >= total_steps:
        final_path = os.path.join(output_dir, "model_pixel_maze.pt")
