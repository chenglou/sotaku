# ES from scratch with a SMOOTH fitness — the last check on replacing the backprop
# bootstrap. exp_es_ce.py differs from the flat-at-chance exp_es_tiny.py in exactly one
# way: fitness is the negative cross-entropy on empty cells instead of the argmax
# cell count. Argmax counting is a staircase — near-random weight nudges move
# probabilities continuously but rarely flip an argmax, so the staircase may hide a
# slope that cross-entropy can see. If this climbs where cell counting was flat, the
# bootstrap can in principle be replaced; if this is also flat, scalar fitness of any
# smoothness lacks the credit assignment ES-from-scratch needs here.
...
CHECKPOINT_PREFIX = "es_ce_checkpoint_step"

CONFIG = {
    'experiment': 'exp_es_ce',
    'd_model': 32,
    'n_heads': 4,
    'd_ff': 128,
    'n_layers': 4,
...
total_steps = 4000        # chained: gens 0-1999 climbed -12.6 to -2.5 nats, still rising
...
def train(output_dir="."):
    ...
    if start_gen >= total_steps:
        ...
        final_path = os.path.join(output_dir, "model_es_ce.pt")
