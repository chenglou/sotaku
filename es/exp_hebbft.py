# Reward-modulated Hebbian FINE-TUNING: same rule as exp_hebbft's scratch sibling, but
# seeded from the tiny backprop bootstrap (228/1000 at 128 iterations) with fitness at
# 128 — the regime where ES climbed at ~0.05 puzzles per generation using 32 rollouts
# per update. This rule gets one rollout per update with per-connection granularity;
# the comparison is fitness gained per rollout.
# Original scratch-variant header follows.
...
CHECKPOINT_PREFIX = "hebbft_checkpoint_step"

CONFIG = {
    'experiment': 'exp_hebbft',
...
total_steps = 1000        # updates; one rollout each
eval_every = 100
lr = 1e-3
baseline_ema = 0.9
fitness_puzzles = 384
...
def train(output_dir="."):
    ...
    if start_step >= total_steps:
        final_path = os.path.join(output_dir, "model_hebbft.pt")
