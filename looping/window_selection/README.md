...
The source checks include the preceding study's launcher, which received [upload-only maintenance](../weight_tying/README.md#running-the-study) after completion. The original code remains at `7eda03c`; do not replace the completed preflight or run records to bypass source checks. New training after code changes needs a new study ID and preflight.
