"""Download committed progress and verify completed diagnostic artifacts."""
    ...
    for key in settings["models"]:
        if f"{key}.log" in entries:
            ...
            lines = [line for line in path.read_text().splitlines()
                     if line.startswith(("PREFLIGHT", "SCREEN", "SELECTED", "ROWS", "LEVEL", "COMPLETE"))]
            print(key, *lines[-4:], sep="\n", flush=True)
        if logs_only or key not in entries:
            continue
        names = {str(Path(entry.path).relative_to(f"{root}/{key}"))
                 for entry in volume.iterdir(f"{root}/{key}", recursive=True)}
        checksums = {}
        if "completed.json" in names:
            ...
            if result["status"] != "complete" or set(result["cases"]) != {"slow", "easy"}:
                raise ValueError("Incomplete completion record")
            checksums = result["sha256"]
        for name in sorted(names):
            if name == "completed.json" or Path(name).suffix not in (".json", ".npz", ".png"):
                continue
            if "rows_" in name or "batch_" in name or "/grid/" in name:
                continue
            download(f"{root}/{key}/{name}", destination / key / name, checksums.get(name))
        print(f"{key}: {'complete; downloaded artifact checksums verified' if checksums else 'running'}", flush=True)
