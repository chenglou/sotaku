"""One detached, resumable screen-and-render pipeline per checkpoint."""

import modal

app = modal.App("sotaku-slow-puzzles-fp64")
...
if modal.is_local():
    ...
    from looping.basin_diagnostics.slow_puzzles.run import SOURCE_PATHS
...
@app.function(image=image, gpu="H200", cpu=8, memory=32768, timeout=12 * 3600,
...
def worker(key, smoke=False):
    ...
    volume.reload()
    from looping.basin_diagnostics.slow_puzzles.run import protocol, run_model
    ...
    try:
        with (root / f"{'smoke_' if smoke else ''}{key}.log").open("a", buffering=1) as log:
            with contextlib.redirect_stdout(Tee(sys.stdout, log)), contextlib.redirect_stderr(Tee(sys.stderr, log)):
                print(f"WORKER key={key} smoke={smoke} root={root}", flush=True)
                return run_model(key, root, callback=volume.commit, smoke=smoke)
    finally:
        volume.commit()


@app.local_entrypoint()
def main(key: str, smoke: bool = False):
