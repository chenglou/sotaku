...
app = modal.App("sotaku-hyperloop-50k")
...
if modal.is_local():
    from looping.hyperloop_50k.common import SOURCE_PATHS
...
def configure():
    ...
    outputs.reload()
    from looping.hyperloop_50k.common import protocol
    settings = protocol()
...
def smoke():
    ...
    from looping.hyperloop_50k.preflight import gpu_preflight
...
def train(arm: str, seed: int):
    ...
    from looping.hyperloop_50k.common import SOURCE_PATHS, run_name
    from looping.hyperloop_50k.evaluate import evaluate_run
...
def main(action: str = "smoke", arm: str = "baseline", seed: int = 20260910):
    from looping.hyperloop_50k.common import run_config
    if action == "smoke":
        call = smoke.spawn()
    elif action == "train":
        run_config(arm, seed)
