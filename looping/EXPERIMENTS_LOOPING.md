...
- Training on randomly sampled later iterations is the recommended default. The standard 20K and 50K runs tested here retained high final accuracy at 1024. The recommended 50K final checkpoint scores 99.12% at 1024 and 98.63% at 4096 in FP32. Its historical BF16 scores were 98.80% at 1024 and 92.98% at 2048.
...
- The highest-scoring July BF16 research checkpoint combines training on later iterations through step 39K with a second future cross-entropy window and calibrated minimum-margin loss through step 50K. Its final weights reached 99.00% at 1024, 98.46% at 2048, and 82.17% at 4096. This staged result has not yet been replicated; later-iteration training alone remains the recommended public recipe.
...
On 80% of batches, cross-entropy is averaged across iterations 1-16. On the other 20%, the model first runs without gradients for a randomly selected number of iterations in `{32, 64, 128, 256, 512}`. It then trains through the next 16 iterations using the same averaged cross-entropy. For example, a 128-iteration initial segment is followed by loss on iterations 129-144. The hidden state and prediction feedback are detached at that boundary, meaning gradients do not flow into the initial segment, also called burn-in. This replaces the ordinary window on selected batches. Historical configurations call the method `late_state_ce`.
