"""Run the N=64 scaled checkpoint's full-horizon evaluation on Modal."""

import modal


app = modal.App("sudoku-scaled-n64-full-eval")
...
def run_evaluation(run_name: str, examples_per_bucket: int):
    import os
    import sys
    ...
    from looping.eval_scaled_n64_full import evaluate_run
...
def main(
    run_name: str = "loop_scaled_n64_trial0",
    examples_per_bucket: int = 5000,
):
    if examples_per_bucket <= 0:
        raise ValueError("examples_per_bucket must be positive")
    call = run_evaluation.spawn(run_name, examples_per_bucket)
    print(f"Spawned scaled N=64 full evaluation: {call.object_id}")
    print(
