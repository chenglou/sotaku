...
The current later-iteration training recipe leaves dropout enabled during the initial iterations that run without gradients, also called burn-in. Inference disables dropout. Does removing dropout only from those initial iterations improve ordinary inference, or does it remove useful regularization?
...
Evaluate final checkpoints, not only the best 1K monitoring result. Record the mean and minimum monitoring scores from steps 40K through 43K, and use the same frozen 25K benchmark at 128/1024/2048/4096 for both members of each pair. Also record puzzles solved and subsequently lost.

Extend both members of each pair to the original 50K endpoint only if both dropout-off runs retain 1024 accuracy within 0.5 percentage points of their paired controls, improve 2048 accuracy by at least 1 point, and do not reduce the minimum 1024 monitoring score by more than 1 point. Preserve the 4K continuation artifacts and use new directories for extensions. A split result calls for another matched random seed, not changing the recommendation. A clear negative result stops this experiment. Any conclusion from these selected high-accuracy checkpoints concerns continuation, not training from scratch.
...
All four branches completed. Both pairs have exactly matching puzzle-order and initial-iteration-count digests, retained in their [training results](../release/validation/dropout). All loop parameters received 4,000 additional optimizer updates. The initial encoder received 3,218 for seed `20260724` and 3,209 for seed `20260730`, because gradients do not reach it on their 782 and 791 later-iteration batches. The original schedule's final LR is `0.00011963356315309796` in all four branches.
...
Keep dropout enabled during the initial gradient-free iterations. Neither pair meets the predeclared extension rule, so no 50K extensions were run. The eight final evaluations retain per-puzzle predictions, independently checked against the pinned dataset. Continuing from selected checkpoints does not establish training-from-scratch reliability.
