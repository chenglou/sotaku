...
from looping.width.common import SOURCE_PATHS, build_model, protocol, run_config, validate_data
...
def export_state(state, path, config, updates, data_identity, source_identity):
    ...
    manifest = {"artifact_type": "sotaku-width-study-inference", "schema_version": 1,
...
def load_export(path, *, device="cpu"):
    ...
    if manifest.get("artifact_type") != "sotaku-width-study-inference" or manifest.get("schema_version") != 1:
        raise ValueError("Not a width study inference export")
