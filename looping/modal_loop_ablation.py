"""Launch one controlled loop ablation on Modal.

Run each arm/trial in a separate detached command, for example:
    modal run --detach looping/modal_loop_ablation.py \
        --arm residual-linear-all --trial 0
"""
...
def run_trial(arm: str, run_name: str, random_seed: int):
    ...
    from looping.exp_loop_ablation import train
...
def main(
    ...
    run_name = f"{run_prefix}_{arm}_trial{trial}"
