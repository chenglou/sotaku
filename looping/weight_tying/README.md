...
The same-width pair has equal transformer matmul FLOPs, depth, puzzle batches, and optimizer updates, not equal parameter count. Optimizer work and memory traffic differ and are measured separately. The parameter-matched pair differs in width, positional representation, and compute; it is not an isolated weight-sharing intervention. Report both comparisons, not one as a substitute for the other.
