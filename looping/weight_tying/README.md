...
Each model performs four transformer blocks per iteration and shares the input encoder, prediction-feedback projection, and output head. `tied` uses the same four blocks at every iteration. `untied_compute` has sixteen independent four-block stages at the same width: 64 distinct transformer blocks in total. Its stages start as copies of the tied model, so the two models initially compute the same function. The copies can subsequently learn different weights. `untied_parameters` also has 64 distinct blocks, but narrows them to width 32 and feedforward width 124, keeping the parameter count within 0.1% of `tied`.
...
After the study, the Modal launcher adopted the shared upload exclusions and guarded its local configuration import. Model and training code are unchanged, but the launcher's source hash differs from the completed runs. Preserve the existing outputs and preflight records; any new training with changed code needs a new study ID and preflight.
...
Collect existing training calls with `python -m looping.weight_tying.watch --registry looping/weight_tying/jobs.json --output LOCAL_RESULT_CACHE.json`. An evaluation registry uses `"action": "evaluate"` and adds `selection` to each job. The collector cannot launch or cancel workers, rejects duplicate identities, and resumes its local cache. Connection errors are read failures, not training failures; verify durable Volume files separately.
