...
The sample contains 60 canonical test puzzles selected with seed 20260811 and balanced across the five rating buckets. Puzzles were assigned before model inference to discovery, validation, and final splits of 20 puzzles each, with four puzzles from each rating bucket in every split. The same whole-puzzle split was used for the four checkpoints from `eval_trajectory_geometry.py`: stable plain, collapsed plain, standalone late-state cross-entropy, and combined late-state cross-entropy plus recheck and margin protection.
...
The final eligible sample has 78 checkpoint-puzzle rows and 17 collapses. All 17 collapses occur in collapsed plain: stable plain has 0/20, collapsed plain has 17/18, late-state CE has 0/20, and combined margin has 0/20. Discovery and validation are even more completely separated by checkpoint: every eligible collapsed-plain puzzle collapses, while no eligible puzzle from the other checkpoints collapses.
