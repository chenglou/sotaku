...
The analysis includes the earlier `existing_trajectory_geometry_v2.json` result for comparison. That diagnostic found compact and smooth per-cell updates but poor transfer of whole-board projections to other puzzles. The new controls use held-out, rating-stratified puzzles and distinguish digit identity from natural cyclic order. Their conclusion agrees with the earlier global-PCA and trajectory-control plots, which show roughly straight or fanning paths rather than a shared helix.
