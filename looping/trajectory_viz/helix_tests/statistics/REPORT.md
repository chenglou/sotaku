...
- Natural-order tests use 9,999 global label permutations, with one mapping shared across all cells, puzzles, iterations, and folds. The one-sided alternative is that the observed natural-order fit is larger. Plus-one Monte Carlo `p` values are corrected by Holm's method across 80 state/output-null tests. Learned-parameter tests form a separate 36-test family.
...
| late-state CE | true | .043 | .086 | .125 | .361 | .346 | .9224 |
| late-state CE | predicted | .051 | .101 | .146 | .419 | .348 | .9148 |
...
| late-state CE | 32 / .250 | 128 / .737 | .148 / .527 |
...
| late-state CE | .125 → .128 → .128 | .146 → .150 → .150 |
...
| checkpoint | incorrect cell-snapshots | state helix `R²`, true / predicted | outside-head helix `R²`, true / predicted |
|---|---:|---:|---:|
| stable plain | 13,885 | .0146 / .0590 | .0029 / .0305 |
| collapsed plain | 13,967 | .0149 / .0558 | .0039 / .0309 |
| late-state CE | 13,388 | .0153 / .0715 | .0051 / .0491 |
...
Projecting out the centered output-head contrast row space removes eight linear directions. It does not rescue natural order:
...
| late-state CE | .120 / .9131 | .141 / .9027 |
| combined margin | .0959 / .4647 | .114 / .4505 |

All output-null Holm-adjusted values are again `1.00`. The projection retains `88.5%`, `96.5%`, `98.8%`, and `98.2%` of cell-centered state energy at iteration 1024 for stable, collapsed, late-state, and combined checkpoints. Sotaku therefore carries redundant digit-category information outside the direct readout contrast span, but that information still does not privilege natural numerical order. See [the output-null permutation nulls](output_null_order_permutations.png).
...
- [Per-cell temporal geometry](../dynamics/REPORT.md) finds real smooth, low-dimensional dynamics and model-specific nonnumeric digit cycles, but no shared natural digit cycle in states, output-null states, output rows, or actual prediction changes.
...
- A linear output-null projection can remove only linear decoder directions. A nonlinear readout could still recover different structure.
