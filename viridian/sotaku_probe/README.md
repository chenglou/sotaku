# Short Sotaku Training And Resume Test

This is the first Viridian test using the actual Sotaku model. It uses `iters.exp_baseline_lr2e3` and its encoding helpers, but feeds a small repo-packaged Sudoku subset so the run can finish quickly.

The B200 path is:

1. upload this `repo/` as a Viridian baseline;
2. submit a B200 runner job that trains a few steps, saves a checkpoint in Sotaku's format, writes a JSONL report, and uploads both to `r2:sotaku-viridian`;
...
This tests the actual model code, checkpoint format, compile order, B200 availability, and the same R2 storage workflow intended for larger runs. It uses packaged data and stubs `datasets.load_dataset` during import, so it does not test Hugging Face dataset download or a long training schedule.
