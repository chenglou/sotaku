...
CHECKPOINT_PREFIX = "testbed_emaslow_d_checkpoint_step"

CONFIG = {
    'experiment': 'exp_testbed_emaslow_d',
    'd_model': 128,
    'd_ff': 512,
    'n_layers': 4,
    'batch_size': 2048,
...
log_name = "exp_testbed_emaslow_d.log"
...
def train(output_dir="."):
    ...
    # Exponential moving average of the weights (decay 0.9999, ~10K-step horizon —
    # the fast 0.999 EMA tracked the live weights into collapse; this one averages over
    # the same span as the checkpoint-averaging pilot that partially recovered a run).
    # The probe evaluates both the live weights and the EMA weights each time.
    ema_params = [p.detach().clone().float() for p in model.parameters()]
    ...
    for step in range(start_step, total_steps):
        ...
        optimizer.step()
        with torch.no_grad():
            for pe, p in zip(ema_params, model.parameters()):
                pe.mul_(0.9999).add_(p.float(), alpha=0.0001)
        ...
        if step % 100 == 0 or step == total_steps - 1:
            ...
            if step % 1000 == 0 and step > 0:
                ...
                with torch.no_grad():
                    for p, pe in zip(model.parameters(), ema_params):
                        p.data.copy_(pe.to(p.dtype))
                ema_1024 = probe_long_horizon(1024)
                with torch.no_grad():
                    for p, b in zip(model.parameters(), backup):
                        p.data.copy_(b)
                log(f"PROBE {step:5d} | 128-iter: {solved_128}/{len(probe_puzzles)} | 1024-iter: {solved_1024}/{len(probe_puzzles)} | ema-1024: {ema_1024}/{len(probe_puzzles)}")

    log("\n" + "="*60)
    log("FINAL RESULTS - exp_testbed_emaslow_d")
    log("="*60)
    results = evaluate_all()
    total_r = results.pop('_total')
    ...
    final_path = os.path.join(output_dir, "model_testbed_emaslow_d.pt")
    state_dict = {k.replace('_orig_mod.', ''): v for k, v in model.state_dict().items()}
    torch.save(state_dict, final_path)
    with torch.no_grad():
        for p, pe in zip(model.parameters(), ema_params):
            p.data.copy_(pe.to(p.dtype))
    ema_path = os.path.join(output_dir, "model_testbed_emaslow_d_avg.pt")
    torch.save({k.replace('_orig_mod.', ''): v for k, v in model.state_dict().items()}, ema_path)
    log(f"EMA model saved: {ema_path}")
