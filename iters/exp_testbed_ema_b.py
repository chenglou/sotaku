...
CHECKPOINT_PREFIX = "testbed_ema_b_checkpoint_step"

CONFIG = {
    'experiment': 'exp_testbed_ema_b',
    'd_model': 128,
    'd_ff': 512,
    'n_layers': 4,
    'batch_size': 2048,
...
log_name = "exp_testbed_ema_b.log"
...
def train(output_dir="."):
    ...
    log("\n" + "="*60)
    log("FINAL RESULTS - exp_testbed_ema_b")
    log("="*60)
    results = evaluate_all()
    total_r = results.pop('_total')
    ...
    final_path = os.path.join(output_dir, "model_testbed_ema_b.pt")
    state_dict = {k.replace('_orig_mod.', ''): v for k, v in model.state_dict().items()}
    torch.save(state_dict, final_path)
    with torch.no_grad():
        for p, pe in zip(model.parameters(), ema_params):
            p.data.copy_(pe.to(p.dtype))
    ema_path = os.path.join(output_dir, "model_testbed_ema_b_avg.pt")
