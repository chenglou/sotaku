# Exact rerun of exp_bs2048_baseline (LR=1.5e-3, previous SOTA, 98.1% at 1024) with a
# fresh unseeded RNG realization. Purpose: test whether the recipe family is robust at
# LR=1.5e-3 while fresh LR=2e-3 runs collapse — i.e. whether 2e-3 sits on the cliff edge.
...
CHECKPOINT_PREFIX = "bs2048_baseline_rerun_checkpoint_step"

CONFIG = {
    'experiment': 'exp_bs2048_baseline_rerun',
    'd_model': 128,
    'd_ff': 512,
    'n_layers': 4,
    'batch_size': 2048,
...
log_name = "exp_bs2048_baseline_rerun.log"
...
def train(output_dir="."):
    ...
    log("\n" + "="*60)
    log("FINAL RESULTS - exp_bs2048_baseline_rerun")
    log("="*60)
    results = evaluate_all()
    total_r = results.pop('_total')
    ...
    final_path = os.path.join(output_dir, "model_bs2048_baseline_rerun.pt")
