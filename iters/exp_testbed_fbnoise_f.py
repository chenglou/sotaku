...
CHECKPOINT_PREFIX = "testbed_fbnoise_f_checkpoint_step"

CONFIG = {
    'experiment': 'exp_testbed_fbnoise_f',
    'd_model': 128,
    'd_ff': 512,
    'n_layers': 4,
    'batch_size': 2048,
...
log_name = "exp_testbed_fbnoise_f.log"
...
class SudokuTransformer(nn.Module):
    ...
    def forward(self, x, return_all=False):
        ...
        for _ in range(n_iterations):
            ...
            # Feedback noise: during training, perturb the logits the model feeds back
            # to itself, so every iteration learns to correct small errors in its own
            # predictions instead of only ever seeing clean self-input.
            if self.training:
                preds = F.softmax(logits + 0.1 * torch.randn_like(logits), dim=-1)
            else:
...
def train(output_dir="."):
    ...
    log("\n" + "="*60)
    log("FINAL RESULTS - exp_testbed_fbnoise_f")
    log("="*60)
    results = evaluate_all()
    total_r = results.pop('_total')
    ...
    final_path = os.path.join(output_dir, "model_testbed_fbnoise_f.pt")
