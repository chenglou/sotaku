# Exact rerun of exp_baseline_lr2e3 with a fresh unseeded RNG realization.
# Purpose: measure run-to-run variance of 1024-iter stability. The Viridian B200
# reproduction collapsed at 1024 test iters; the canonical headline is a best-run result.
...
CHECKPOINT_PREFIX = "baseline_lr2e3_rerun_checkpoint_step"

CONFIG = {
    'experiment': 'exp_baseline_lr2e3_rerun',
    'd_model': 128,
    'd_ff': 512,
    'n_layers': 4,
    'batch_size': 2048,
...
log_name = "exp_baseline_lr2e3_rerun.log"
...
def train(output_dir="."):
    ...
    log("\n" + "="*60)
    log("FINAL RESULTS - exp_baseline_lr2e3_rerun")
    log("="*60)
    results = evaluate_all()
    total_r = results.pop('_total')
    ...
    final_path = os.path.join(output_dir, "model_baseline_lr2e3_rerun.pt")
