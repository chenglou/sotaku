...
CHECKPOINT_PREFIX = "testbed_ema_c_checkpoint_step"

CONFIG = {
    'experiment': 'exp_testbed_ema_c',
    'd_model': 128,
    'd_ff': 512,
    'n_layers': 4,
    'batch_size': 2048,
...
log_name = "exp_testbed_ema_c.log"
...
def train(output_dir="."):
    ...
    # Exponential moving average of the weights (decay 0.999, ~1K-step horizon).
    ...
    for step in range(start_step, total_steps):
        ...
        with torch.no_grad():
            for pe, p in zip(ema_params, model.parameters()):
                pe.mul_(0.999).add_(p.float(), alpha=0.001)
    ...
    log("\n" + "="*60)
    log("FINAL RESULTS - exp_testbed_ema_c")
    log("="*60)
    results = evaluate_all()
    total_r = results.pop('_total')
    ...
    final_path = os.path.join(output_dir, "model_testbed_ema_c.pt")
    state_dict = {k.replace('_orig_mod.', ''): v for k, v in model.state_dict().items()}
    torch.save(state_dict, final_path)
    with torch.no_grad():
        for p, pe in zip(model.parameters(), ema_params):
            p.data.copy_(pe.to(p.dtype))
    ema_path = os.path.join(output_dir, "model_testbed_ema_c_avg.pt")
