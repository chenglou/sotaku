...
Practical upshot, revised: ES reliably polishes stable models (96.0 to 96.5, and flat through 2048) and rescues near-misses, but does not resurrect deep collapses from their final weights. Since failed runs almost always pass through much better mid-training checkpoints (see the harvest statistic above), the untested recovery path for deep failures is ES from the best checkpoint a run produced rather than from its final state. Other open directions: tune at 2048+ to deepen a repaired basin, and whether ES can lift a stable model past February's 98.9%.
