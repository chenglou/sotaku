...
What this rules out as the cause: the recreated training loop (it computes the same math, and both loop implementations produced both outcomes), the platform and torch version (both platforms produced both outcomes), the choice between LR=1.5e-3 and 2e-3 (1.5e-3 collapsed twice even without interruption), seeding, and mid-run interruption (uninterrupted runs collapse at the same rate as interrupted-then-resumed ones). What remains: in July 2026, a fresh run ends up stable at 1024 iterations roughly 2 times in 12 (~17%) with everything we control held fixed, versus February's 4 of 4. That February-vs-July split is the one statistically significant pattern (p≈0.008) and is unexplained — candidate causes are GPU-fleet changes between February and July (driver versions were never logged in February, the Modal apps are garbage-collected, so this is unrecoverable self-service; only Modal support could answer) or February simply drawing four successes in a row by chance, which is made less improbable by its four runs being four different configs with different success rates. A volume-and-transcript audit added three caveats about February worth recording. The February runs requested gpu="H200" but never verified the hardware at runtime (Modal may auto-upgrade GPU types), so even the original silicon identity is unconfirmed. A possible Modal image rebuild on Feb 10 — between the first two and last two of the four February runs — was noticed at the time ("huh are we building differently than before") but never resolved, so "same image" across all four is inferred, not proven. And the only pre-July same-config repeat anywhere in the record is exp_faster (Feb 5), identical in hyperparameters to cosine/exp_cosine_50k: 81.2% vs 82.8% at 16 iterations — a 1.6pp gap that shows run-to-run variance was already visible at 16 iterations, though nothing from that era measured it at 1024. One relevant fact from probing both fleets on 2026-07-02: Modal's H200 fleet and the external B200 provider (whose compute is also Modal) both run driver 580.95.05 / CUDA 13.0 — the two July "platforms" were one environment, so the study's platform axis carried no independent information. Every training wrapper now logs `nvidia-smi` and torch versions per run, so this ambiguity cannot recur.

Two facts from evaluating intermediate checkpoints of the July runs. First, a model's 1024-iteration accuracy swings wildly during training while its training loss stays smooth — one run that ended collapsed read 18% → 92.5% → 2.2% at 1024 iterations across steps 32K → 34K → 36K. Second, the outcome is not decided until the very end of the learning-rate decay: runs both gain and lose their long-iteration stability during the final 10K steps. One run read 7.2% at step 40K and finished at 96.0%; another read 92.8% at step 40K and finished at 31.2%. Across the four uninterrupted July runs, two held 90%+ at their step-40K checkpoint but only one still had it at step 50K — so evaluating and keeping mid-training checkpoints doubled the number of usable models at no training cost. A follow-up test (three clean runs each of exp_3phase_40k and exp_bs2048_mixed, July 2) refined this picture. The idea that exp_3phase_40k is safe because it "stops before the dangerous stretch" was wrong — its cosine schedule completes at 40K, so it has its own end-of-decay stretch, and all three reruns failed at 1024 iterations (0.3% / 1.4% / 24.6%); February's 95.9% was another low-probability success, not a property of the config. exp_bs2048_mixed, however, is the one configuration that has never collapsed hard: across its four runs ever (February 97.3%, July 88.8% / 72.1% / 95.5%), the worst 1024-iteration result is 72%. Every curriculum-trained config either stays above 90% or crashes below 30%, while mixed sampling degrades gently — suggesting the reverse curriculum plays a role in the crash mode itself, though four runs is thin evidence.
...
Moved to [stabilize/EXPERIMENTS_STABILIZE.md](../stabilize/EXPERIMENTS_STABILIZE.md): a 20K-step testbed compared weight averaging, feedback noise, and gradient-free burn-in against the instability. Headline: 128-iteration burn-in stabilizes 6 of 7 runs at a ~90% ceiling, and saving checkpoints against an in-training probe roughly triples the yield of usable models on any config.

## Evolution-Strategies Fine-Tuning (July 2026)

Moved to [es/EXPERIMENTS_ES.md](../es/EXPERIMENTS_ES.md): forward-only fine-tuning on the 1024-iteration solve rate itself. Headline: any checkpoint with healthy 128-iteration accuracy fine-tunes to 94-96% at 1024 in about 90 forward-only GPU-minutes, even from runs that ended fully collapsed; weights broken at short horizons too are unrescuable, and burn-in-trained weights resist ES. This supersedes burn-in as the default recipe.
