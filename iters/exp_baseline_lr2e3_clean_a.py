# Clean-run test a: exact exp_baseline_lr2e3 recipe, fresh unseeded draw, must run
# uninterrupted start to finish. Purpose: test whether mid-anneal interruption+resume
# (present in every post-February run) is what separates them from February's 4/4 settles.
...
CHECKPOINT_PREFIX = "baseline_lr2e3_clean_a_checkpoint_step"

CONFIG = {
    'experiment': 'exp_baseline_lr2e3_clean_a',
    'd_model': 128,
    'd_ff': 512,
    'n_layers': 4,
    'batch_size': 2048,
...
log_name = "exp_baseline_lr2e3_clean_a.log"
...
def train(output_dir="."):
    ...
    log("\n" + "="*60)
    log("FINAL RESULTS - exp_baseline_lr2e3_clean_a")
    log("="*60)
    results = evaluate_all()
    total_r = results.pop('_total')
    ...
    final_path = os.path.join(output_dir, "model_baseline_lr2e3_clean_a.pt")
