# Clean-run test a: exact exp_bs2048_baseline (LR=1.5e-3) recipe, fresh unseeded draw,
# must run uninterrupted start to finish. Purpose: test whether mid-anneal interruption
# separates post-February runs (1/5 settled) from February's clean 4/4.
...
CHECKPOINT_PREFIX = "bs2048_baseline_clean_a_checkpoint_step"

CONFIG = {
    'experiment': 'exp_bs2048_baseline_clean_a',
    'd_model': 128,
    'd_ff': 512,
    'n_layers': 4,
    'batch_size': 2048,
...
log_name = "exp_bs2048_baseline_clean_a.log"
...
def train(output_dir="."):
    ...
    log("\n" + "="*60)
    log("FINAL RESULTS - exp_bs2048_baseline_clean_a")
    log("="*60)
    results = evaluate_all()
    total_r = results.pop('_total')
    ...
    final_path = os.path.join(output_dir, "model_bs2048_baseline_clean_a.pt")
