# The full exp_baseline_lr2e3 recipe (50K steps) plus 128-iteration gradient-free
# burn-in on 20% of steps — the intervention that went 4/4 stable at 1024 test
# iterations on the 20K testbed (baseline: 2/7). This is the confirmation run at the
# real schedule; it keeps the in-training probe (every 2,000 steps) so the stability
...
CHECKPOINT_PREFIX = "lr2e3_burnin128_a_checkpoint_step"

CONFIG = {
    'experiment': 'exp_lr2e3_burnin128_a',
    'd_model': 128,
    'd_ff': 512,
    'n_layers': 4,
    'batch_size': 2048,
...
log_name = "exp_lr2e3_burnin128_a.log"
...
class SudokuTransformer(nn.Module):
    ...
    def forward(self, x, return_all=False, init_state=None):
        batch_size = x.size(0)
        device = x.device
        rope_cos = ROPE_COS.to(device)
        rope_sin = ROPE_SIN.to(device)

        # init_state lets training start the supervised window from a state produced
        # by gradient-free burn-in iterations, so the model learns to behave on the
        # states it reaches deep into its own trajectory, not only on fresh starts.
        if init_state is None:
            h_prev = self.initial_encoder(x)
            preds = torch.zeros(batch_size, 81, 9, device=device)
        else:
            h_prev, preds = init_state
...
def train(output_dir="."):
    ...
    # Gradient-free burn-in, 128 iterations: one 32-iteration block compiled once and
    # called four times, so the horizon is a multiplier rather than a new graph.
    # Dose-response test: 32 burn-in iterations protected 128-iteration accuracy but
    # not 1024; if protection follows the trained horizon, 128 should reach further.
    orig_module = getattr(model, '_orig_mod', model)

    def _burnin_chunk(h_prev, preds):
        rope_cos = ROPE_COS.to(h_prev.device)
        rope_sin = ROPE_SIN.to(h_prev.device)
        for _ in range(32):
            h = h_prev + orig_module.pred_proj(preds)
            for layer in orig_module.layers:
                h = layer(h, rope_cos, rope_sin)
            h_prev = h
            preds = F.softmax(orig_module.output_head(h), dim=-1)
        return h_prev, preds

    burnin_chunk = torch.compile(_burnin_chunk)

    def burnin_fn(x):
        h_prev = orig_module.initial_encoder(x)
        preds = torch.zeros(x.size(0), 81, 9, device=x.device)
        for _ in range(4):
            h_prev, preds = burnin_chunk(h_prev, preds)
        return h_prev, preds
    ...
    def compute_loss(x_batch, t_batch, init_state=None):
        all_logits = model(x_batch, return_all=True, init_state=init_state)
        mask = x_batch[:, :, 0]
    ...
    for step in range(start_step, total_steps):
        ...
        if random.random() < 0.2:
            with torch.no_grad(), torch.autocast('cuda', dtype=torch.bfloat16):
                bh, bp = burnin_fn(x_batch)
            init_state = (bh.detach(), bp.detach())

        optimizer.zero_grad()
        with torch.autocast('cuda', dtype=torch.bfloat16):
            loss, all_logits, mask, t_batch = compute_loss(x_batch, t_batch, init_state)
        loss.backward()
        optimizer.step()
    ...
    log("\n" + "="*60)
    log("FINAL RESULTS - exp_lr2e3_burnin128_a")
    log("="*60)
    results = evaluate_all()
    total_r = results.pop('_total')
    ...
    final_path = os.path.join(output_dir, "model_lr2e3_burnin128_a.pt")
