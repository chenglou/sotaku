...
The headline number came from the best of several unseeded training runs, and retraining does not reliably reproduce it. In a July 2026 study, 12 fresh runs of the two known-good configurations (8 of this one, 4 of the LR=1.5e-3 predecessor; same code, data, and dependencies) produced a model that stays accurate at 1024 test-time iterations only 2 times out of 12. The other runs looked completely normal at 16 iterations but degraded badly when iterated longer. To get a model above 98%, train several runs of this config, evaluate each saved checkpoint at 1024 iterations, and keep the best checkpoint (3 of 9 runs on record produced 92%+). If you would rather have one dependable run than the highest ceiling, `iters/exp_bs2048_mixed.py` has never finished below 72% and reaches 95%+ about half the time. The Reproducibility section of `iters/EXPERIMENTS_ITERS.md` has the full study and both recipes. Evaluating a fixed checkpoint, on the other hand, is deterministic and reproduces exactly (test subsampling in `iters/eval_more_iters.py`).
