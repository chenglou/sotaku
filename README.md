...
The headline number is a best-of-several-runs result. Training this config is noisy: with identical code and data, only about 1 run in 4 ends up stable at 1024 test-time iterations, and the rest degrade at long iteration counts despite normal 16-iteration accuracy. If you're retraining, `iters/EXPERIMENTS_ITERS.md` (Reproducibility and Stabilization sections) has the details and two recipes: gradient-free burn-in plus keeping the best in-training checkpoint lands at 84-90% every time, and repeated plain runs with checkpoint selection reach 92%+ about a third of the time. Evaluating a fixed checkpoint, on the other hand, is deterministic and reproduces exactly (test subsampling in `iters/eval_more_iters.py`).
